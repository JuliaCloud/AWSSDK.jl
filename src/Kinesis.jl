...
[CreateStream](@ref) has a limit of 5 transactions per second per account.
...
`ResourceInUseException`, `ResourceNotFoundException` or `InvalidArgumentException`.
...
**Note:** Amazon Kinesis might continue to accept data read and write operations, such as [PutRecord](@ref), [PutRecords](@ref), and [GetRecords](@ref), on a stream in the `DELETING` state until the stream deletion is complete.
...
[DeleteStream](@ref) has a limit of 5 transactions per second per account.
...
For more information, see [Monitoring the Amazon Kinesis Streams Service with Amazon CloudWatch](http://docs.aws.amazon.com/kinesis/latest/dev/monitoring-with-cloudwatch.html) in the *Amazon Kinesis Streams Developer Guide*.
...
The size of the data returned by [GetRecords](@ref) varies depending on the utilization of the shard. The maximum size of data that [GetRecords](@ref) can return is 10 MB. If a call returns this amount of data, subsequent calls made within the next 5 seconds throw `ProvisionedThroughputExceededException`. If there is insufficient provisioned throughput on the shard, subsequent calls made within the next 1 second throw `ProvisionedThroughputExceededException`. Note that [GetRecords](@ref) won't return any data when it throws an exception. For this reason, we recommend that you wait one second between calls to [GetRecords](@ref); however, it's possible that the application will get exceptions for longer than 1 second.
...
You must specify the shard iterator type. For example, you can set the `ShardIteratorType` parameter to read exactly from the position denoted by a specific sequence number by using the `AT_SEQUENCE_NUMBER` shard iterator type, or right after the sequence number by using the `AFTER_SEQUENCE_NUMBER` shard iterator type, using sequence numbers returned by earlier calls to [PutRecord](@ref), [PutRecords](@ref), [GetRecords](@ref), or [DescribeStream](@ref). In the request, you can specify the shard iterator type `AT_TIMESTAMP` to read records from an arbitrary point in time, `TRIM_HORIZON` to cause `ShardIterator` to point to the last untrimmed record in the shard in the system (the oldest data record in the shard), or `LATEST` so that you always read the most recent data in the shard.
...
If a [GetShardIterator](@ref) request is made too often, you receive a `ProvisionedThroughputExceededException`. For more information about throughput limits, see [GetRecords](@ref), and [Streams Limits](http://docs.aws.amazon.com/kinesis/latest/dev/service-sizes-and-limits.html) in the *Amazon Kinesis Streams Developer Guide*.

If the shard is closed, [GetShardIterator](@ref) returns a valid iterator for the last sequence number of the shard. Note that a shard can be closed as a result of using [SplitShard](@ref) or [MergeShards](@ref).

[GetShardIterator](@ref) has a limit of 5 transactions per second per account per open shard.
...
[ListStreams](@ref) has a limit of 5 transactions per second per account.
...
If you try to operate on too many streams in parallel using [CreateStream](@ref), [DeleteStream](@ref), `MergeShards` or [SplitShard](@ref), you will receive a `LimitExceededException`.
