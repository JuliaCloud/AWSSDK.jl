...
```
 filter = [
        "namePattern" =>  ::String,
        "states" =>  ["CREATED", "START_DATA_COLLECTION_PENDING", "START_DATA_COLLECTION_IN_PROGRESS", "COLLECTING_DATA", "STOP_DATA_COLLECTION_PENDING", "DATA_COLLECTED", "START_EVALUATING_RULES_PENDING", "EVALUATING_RULES", "FAILED", "ERROR", "COMPLETED" or "COMPLETED_WITH_ERRORS", ...],
        "durationRange" =>  [
            "minSeconds" =>  ::Int,
            "maxSeconds" =>  ::Int
        ],
...
Registers the IAM role that Amazon Inspector uses to list your EC2 instances at the start of the assessment run or when you call the [PreviewAgents](@ref) action.

# Arguments

## `roleArn = ::String` -- *Required*
The ARN of the IAM role that Amazon Inspector uses to list your EC2 instances during the assessment run or when you call the [PreviewAgents](@ref) action.
...
"""
    ...
    inspector([::AWSConfig], "StopAssessmentRun", assessmentRunArn=)
